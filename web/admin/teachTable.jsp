...
<head>
    ...
    <script language="javascript" for="window" event="onload">
        function MakeForm() {
            ...
            var resState =
            <%=request.getAttribute("resState") != null%>
            if(state1){
                if (resState) {
                    layer.msg('密码重置成功!', {icon: 1, time: 2000});
                }
                else {
                    layer.msg('密码重置失败!', {icon: 2, time: 2000});
                }
            }
            if(state2){
                if (delState) {
            ...
            if (flag) {
                ...
                form1.method = "POST";
                // form 提交路径
                form1.action = "/TeachListServlet_admin";
                // 对该 form 执行提交
                form1.submit();
                // 删除该 form
                                ...
                                <table class="table table-striped">
                                    ...
                                    <%
                                        if (request.getAttribute("teachList") != null) {
                                            ...
                                            for (int i = 0, len = teachList.size(); i < len; i++) {
                                    %>
                                        <tr>
                                            <td><%=teachList.get(i).gettId()%></td>
                                            <td><%=teachList.get(i).getName()%></td>
                                            <td><%=teachList.get(i).gettId()%></td>
                                            <td><%=teachList.get(i).getAddress()%></td>
                                            <td>
                                                <button class="btn btn-primary btn-fill "
                                                        onclick="reset(<%=teachList.get(i).gettId()%>)">
                                                    重置密码
                                                </button>
                                                <button class="btn btn-danger btn-fill " onclick="del(<%=teachList.get(i).gettId()%>)">删除
                                                </button>
                                            </td>
                                        </tr>
                                    <%
...
<script type="text/javascript">
    ...
    function del(id) {
        layer.confirm('确认要删除这位教练吗？', function (index) {
            ...
            form1.method = "POST";
            // form 提交路径
            form1.action = "/DelTeachServlet_admin";
            // 对该 form 执行提交
            form1.submit();
            // 删除该 form
            document.body.removeChild(form1);
    ...
    function reset(id) {
        layer.confirm('确认要重置密码？', function (index) {
            ...
            form1.method = "POST";
            // form 提交路径
            form1.action = "/ResetTeachPwdServlet_admin";
            // 对该 form 执行提交
            form1.submit();
            // 删除该 form
            document.body.removeChild(form1);
