...
<%@ page import="com.group.bodyBuilding.vo.Course" %>
...
<head>
    ...
    <script language="javascript" for="window" event="onload">
        function MakeForm() {var state =
            <%=request.getAttribute("state")%>
            var selectState =
            <%=request.getAttribute("selectState")%>
            if(selectState){
                if (state) {
            ...
            %>--%>
            var flag =
            <%=(request.getAttribute("courseList") == null)%>
            if (flag) {
                // 创建一个 form
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            <div class="header">
                                <h4 class="title">课程列表</h4><input type="hidden" value="${SelectTeachInfo}"
                                                                  id="SelectTeachInfo">
                            ...
                            <div class="content table-responsive table-full-width">
                                <table class="table table-striped">
                                    ...
                                    %>
                                    <tr>
                                        <td><%=courseList.get(i).getcId()%>
                                        </td>
                                        <td><%=courseList.get(i).getcName()%>
                                        </td>
                                        </td>
                                        <td><%=courseList.get(i).gettId()%>
                                        </td>
                                        <td>
                                            <button class="btn btn-primary btn-fill " onclick="selectTeach(<%=courseList.get(i).getcId()%>)">
                                                选择课程
                                            </button>
                                        </td>
                                    </tr>
...
<script type="text/javascript">
    function selectTeach(cId) {
        ...
        layer.confirm('确认选择课程吗？', function (index) {
            ...
            form1.method = "POST";
            // form 提交路径
            form1.action = "/SelectCourseServlet_user";
            // 对该 form 执行提交
            form1.submit();
            // 删除该 form
            document.body.removeChild(form1);
